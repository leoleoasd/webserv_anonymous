...
class SlimeRouter:
    ...
    def _setup_routes(self):
        ...
        self.app.get("/get_router_load")(self.get_router_load)
    ...
    async def _health_check_loop(self):
        ...
        while True:
            try:
                ...
                for url, is_healthy in results:
                    if not is_healthy:
                        ...
                        if failures >= threshold:
                            logger.warning(f"[slime-router] Worker {url} failed {threshold} consecutive health checks. Marking as DEAD.")
                ...
                logger.debug(f"[slime-router] Health check complete. {len(self.worker_request_counts) - len(self.dead_workers)} workers healthy.")
    ...
    async def proxy(self, request: Request, path: str):
        """Fully streaming proxy — no JSON parse/encode, no buffering.

        Both the request body (e.g. 80k token IDs) and the response body
        (logprobs, token data) are streamed through chunk-by-chunk so the
        router event loop is never blocked by large payloads.
        """
        ...
        # Forward response headers, dropping hop-by-hop / framing headers
        # (StreamingResponse will set its own transfer-encoding)
        response_headers = {k: v for k, v in upstream.headers.items() if k.lower() not in self._STRIP_RESPONSE_HEADERS}
    ...
    async def add_worker(self, request: Request):
        ...
        if not worker_url:
            return JSONResponse(status_code=400, content={"error": "worker_url is required (use query ?url=... or JSON body)"})
    ...
    async def get_router_load(self, request: Request):
        """Return per-worker in-flight request counts and dead worker list."""
        return {
