...
Override config: python -m rl_web_agent.entrypoints.agent environment.recording.enabled=true
"""
...
async def run_agent_task(cfg: DictConfig):
    ...
    try:
        ...
        # Auto-configure screenshot_path if VLM mode is enabled
        if cfg.agent.get("vlm", False):
            if not hasattr(cfg.environment, "screenshot_path") or cfg.environment.get("screenshot_path") is None:
                # Create screenshots directory in the Hydra output directory
                screenshots_dir = Path.cwd() / "screenshots"
                screenshots_dir.mkdir(parents=True, exist_ok=True)
                cfg.environment.screenshot_path = str(screenshots_dir)
                logger.info(f"VLM mode enabled - auto-configured screenshot_path: {screenshots_dir}")
            else:
                logger.info(f"VLM mode enabled - using configured screenshot_path: {cfg.environment.screenshot_path}")
        ...
        if hasattr(cfg, "task_config") and cfg.task_config:
            ...
            test_task_path = project_root / "dependencies" / "webarena" / "config_files" / "102.json"
...
def main(cfg: DictConfig) -> None:
    ...
    logger.info("Starting WebAgent with configuration:")
    ...
    logger.info(f"  VLM mode: {cfg.agent.get('vlm', False)}")
    ...
    try:
        asyncio.run(run_agent_task(cfg))
