...
async def generate_rollout_async(args, rollout_id: int, data_buffer) -> RolloutFnTrainOutput:
    ...
    while len(data) < target_data_size:
        ...
        for group_id in available_ids:
            ...
            # Add samples that passed the dynamic filter
            data.append(group)
            processed_any = True
        ...
        if current_time - last_progress_time > no_progress_timeout:
            print(f"Warning: No progress for {no_progress_timeout}s. Queue size: {worker.get_queue_size()}, Collected: {len(data)}/{target_data_size}")
